...
Game.prototype.addShields = function() {
  for (let i = 0, x = .05; i < 5; i++, x += 0.2) {
    ...
    let shieldPosY = this.canvasSize[1] * .78;
...
Ship.prototype.killScore = function() {
  ...
  if (this.name === 'grunt') {
    return this.game.blockInfo.options.avg_transaction_sizes[2] + (-0.15 + 0.3 * Math.random()) * this.game.blockInfo.options.avg_transaction_sizes[2];
  } else if (this.name === 'soldier') {
      return this.game.blockInfo.options.avg_transaction_sizes[0] + (-0.15 + 0.3 * Math.random()) * this.game.blockInfo.options.avg_transaction_sizes[0];
  } else if (this.name === 'invader') {
      return this.game.blockInfo.options.avg_transaction_sizes[1] + (-0.15 + 0.3 * Math.random()) * this.game.blockInfo.options.avg_transaction_sizes[1];
